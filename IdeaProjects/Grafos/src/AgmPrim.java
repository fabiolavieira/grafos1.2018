...
public class AgmPrim {
    ...
    public void obterAgm(int raiz) throws Exception {
        ...
        this.p = new int[n]; // @{\it peso dos v\'ertices}@
    ...
    public List<AgmAresta> retornaArestas() {
        List<AgmAresta> arestas = new ArrayList<AgmAresta>();
        for (int u = 0; u < this.p.length; u++)
            if (this.antecessor[u] != -1) {
                arestas.add(new AgmAresta(antecessor[u], u, peso(u)));
            }
        return arestas;
    }
}